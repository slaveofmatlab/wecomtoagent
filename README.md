...
### 第一步：把当天的三份 Excel 放进对应文件夹

| Excel | 说明 | 放在哪个文件夹 |
|---|---|---|
| 销售订单全链路.xlsx | 当天即可 | `示例数据/` |
| 待转单.xlsx | **推荐用最近7天全量导出**（覆盖当天往前7天） | `示例数据/` |
| 企业微信AI转单推进表.xlsx | 当天最新版 | `basicData/` 或当天日期目录 |

> **重要**：待转单建议用「最近7天全量导出」而非单天快照。因为待转单状态会随时间变化（未转→已转），7天全量导出一份文件就包含所有活跃订单的最新状态。

文件名不用完全一致，脚本按"文件名里包含哪几个字"去找（如含"销售订单""待转单""推进表"即可），旧的那份删掉避免重复。
